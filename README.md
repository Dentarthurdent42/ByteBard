...
- **Tracking toggles**: **✋ HANDS** and **🧍 POSE** in the header (once the camera is on) switch each model off outright. Hand tracking costs roughly twice what pose does and is normally the frame-rate bottleneck, so this is the bluntest lever available. With both on the two models alternate frames; with one off, **the other runs every frame** rather than idling on its turn. Dev mode's **MODELS** panel adds finer control — pose model size, `GPU`/`CPU` delegate (which applies to *both* models), and a **HANDS** count, since the landmark stage runs once per detected hand and tracking one is close to half the hand cost.
